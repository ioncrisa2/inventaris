    ...
    <x-app-page>
        ...
        <div class="row g-3 mb-4">
            <div class="col-sm-6 col-xl-2">
                <x-stat-card class="attendance-stat-card--primary" icon="bi-person-check" label="Hadir"
                    :value="$totalHadir" variant="success" />
            </div>
            <div class="col-sm-6 col-xl-2">
                <x-stat-card :class="(int) $totalIzin === 0 ? 'attendance-stat-card--zero' : ''" icon="bi-envelope-check"
                    label="Izin" :value="$totalIzin" variant="warning" />
            </div>
            <div class="col-sm-6 col-xl-2">
                <x-stat-card :class="(int) $totalSakit === 0 ? 'attendance-stat-card--zero' : ''" icon="bi-heart-pulse"
                    label="Sakit" :value="$totalSakit" variant="info" />
            </div>
            <div class="col-sm-6 col-xl-2">
                <x-stat-card :class="(int) $totalCuti === 0 ? 'attendance-stat-card--zero' : ''" icon="bi-calendar2-week"
                    label="Cuti" :value="$totalCuti" />
            </div>
            <div class="col-sm-6 col-xl-2">
            ...
            <div class="col-sm-6 col-xl-2">
                <x-stat-card :class="(int) $totalAlpha === 0 ? 'attendance-stat-card--zero' : ''" icon="bi-person-x"
                    label="Alpha" :value="$totalAlpha" variant="danger" />
            </div>
        </div>
                    ...
                    @foreach ($mingguKalender as $minggu)
                        @foreach ($minggu as $cell)
                            ...
                            <div class="{{ $kondisiKelas }}">
                                @if ($bisaDiklik)
                                    <button type="button" class="calendar-cell-button" data-bs-toggle="modal"
    ...
    <x-modal-form id="modalAbsensi" :data-auto-show-modal="$errors->any()" :data-libur-allowed-statuses="json_encode(\App\Models\Absensi::LIBUR_ALLOWED_STATUSES)" dialog-class="modal-dialog-centered"
        ...
        <x-slot:header>
            <div>
                ...
                <div class="text-muted small" id="modalAbsensiTanggalLabel">
                    {{ old('tanggal') ? \Illuminate\Support\Carbon::parse(old('tanggal'))->translatedFormat('l, d F Y') : '' }}
                </div>
            </div>
        </x-slot:header>
